...
> 点每个标题左边的小三角，可以把那一节折起来 —— 只读你此刻想读的。

# 输入方式

把一份 markdown 交给 miru 有四种方式：

- **粘贴**一段 markdown 文本（全局 `Cmd+V` / `Ctrl+V`，或右下角菜单）
- **拖入**一个 `.md` 文件
- 用右下角菜单的**打开文件**选择本地文档
- 在右下角菜单输入一个允许跨域读取的**原始 URL**
...
把任意 markdown 交给 miru：粘贴、拖入、打开文件、或在右下角 `+` 里贴一个原始 URL。miru 会立即把它变成一份可读的文档。想了解更多，看看 [CommonMark 教程](https://commonmark.org/help/)。
